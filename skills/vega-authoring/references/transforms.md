...
Purpose: Dataflow transforms, ordering concerns, and derived-field behavior.
...
Source URL: https://vega.github.io/vega/docs/transforms/
...
*   [`dotbin`](dotbin) - Perform density binning for dot plot construction. ≥ 5.7
...
*   [`flatten`](flatten) - Map array-typed fields to data objects, one per array entry. ≥ 3.1
...
*   [`kde`](kde) - Estimate smoothed densities for numeric values. ≥ 5.4
...
*   [`loess`](loess) - Fit a smoothed trend line using local regression. ≥ 5.4
*   [`lookup`](lookup) - Extend data objects by looking up key values on another stream.
*   [`pivot`](pivot) - Pivot unique values to new aggregate fields. ≥ 3.2
*   [`project`](project) - Generate derived data objects with a selected set of fields.
*   [`quantile`](quantile) - Calculate sample quantile values over an input data stream. ≥ 5.7
*   [`regression`](regression) - Fit regression models to smooth and predict values. ≥ 5.4
...
*   [`timeunit`](timeunit) - Discretize date-time values into time unit bins. ≥ 5.8
...
*   [`heatmap`](heatmap) - Generate heatmap images for raster grid data. ≥ 5.8
*   [`isocontour`](isocontour) - Generate level set contours for raster grid data. ≥ 5.8
*   [`kde2d`](kde2d) - Estimate 2D densities as output raster grids. ≥ 5.8
...
*   [`label`](label) - Compute text position and opacity to label a chart. ≥ 5.16
...
In addition to the above, custom transformations can also be added to Vega as part of its Extensibility API. See the [Transformations](https://vega.github.io/vega/docs/api/extensibility/#transform) section of the API documentation.
