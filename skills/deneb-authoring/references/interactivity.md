...
Purpose: Selection, highlighting, and Power BI interaction behavior.

Source URLs:
- https://deneb.guide/docs/interactivity-overview

## Interactivity Features - An Overview | Deneb

Source URL: https://deneb.guide/docs/interactivity-overview
...
```
// DAX QueryDEFINE    VAR __DS0Core =        SUMMARIZECOLUMNS (            'financials'[Country],            "Sumv_Sales", CALCULATE ( SUM ( 'financials'[ Sales] ) )        )    VAR __DS0PrimaryWindowed =        TOPN ( 10001, __DS0Core, 'financials'[Country], 1 )EVALUATE__DS0PrimaryWindowedORDER BY 'financials'[Country]
```
...
*   `__selected__` - whether this particular row context is selected for [cross-filtering](/docs/interactivity-selection).

These may become visible to you when [inspecting a datum using tooltips](/docs/interactivity-tooltips#debugging-with-tooltips) or similar, but they aren't currently supported for use outside of anything Deneb does for you.

#### Additional Cross-Highlighting Fields for Measures

The following additional entries will get created for all measures in your dataset in the event of having [cross-highlighting](/docs/interactivity-highlight) enabled on your visual:
...
These are covered in more detail on the [Cross-Highlighting](/docs/interactivity-highlight#special-__highlight-fields) page.
...
*   Default tooltips are different to report page or modern tooltips.
    ...
    *   Tooltip functionality within Deneb is covered in much more detail on the [Tooltips](/docs/interactivity-tooltips) page.
